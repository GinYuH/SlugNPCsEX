...
{
    [AutoloadHead]
    public class DevourerSlug : ModNPC
	{
        public override string Texture => "SlugNPCsEX/NPCs/KeratostomSlug_Devourer";
        ...
        public override bool CanTownNPCSpawn(int numTownNPCs)/* tModPorter Suggestion: Copy the implementation of NPC.SpawnAllowed_Merchant in vanilla if you to count money, and be sure to set a flag when unlocked, so you don't count every tick. */
        {
            for (int k = 0; k < 255; k++)
            {
                ...
                if (NPC.downedMoonlord)//(player.inventory.Any(item => item.type == ItemID.PearlwoodSword) && !Main.npc.Any(npc => npc.type == NPC.type))
                {
                    return true;
                }
            }
